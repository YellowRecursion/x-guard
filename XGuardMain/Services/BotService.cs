                    ...
                    case UpdateType.Message:
                        {
                            ...
                            if (message.Text.StartsWith("/pause") && await IsModerator(client, chat, user))
                            {
                                ...
                                Program.DetectionService.DisableTimer = timer;
                            ...
                            {
                                Program.DetectionService.DisableTimer = 0;
                            ...
                            if (message.Text == "/status")
                            {
                                ...
                                sb.AppendLine($"Статус XGuard ({(Program.DetectionService.DisableTimer > 0 ? "приостановлен" : "работает")}):");
                                sb.AppendLine($"Таймер до возобновления работы: {Program.DetectionService.DisableTimer} с.");
                                sb.AppendLine($"Время без обнаружений NSFW: {Program.DetectionService.NoDetectionsTimer} с.");
                                sb.AppendLine($"Переодичность скриншотов: {Program.DetectionService.DetectionRate} мс.");
                ...
                try
                {
                    ...
                    using (Stream stream = System.IO.File.OpenRead(screenshotPath))
                    {
                        foreach (var chatId in chatIds)
                        {
                            ...
                            {
                                 firstMessage = await _botClient.SendPhotoAsync(chatId, photo: InputFile.FromStream(stream, "image.png"), hasSpoiler: true, caption: caption);
                            }
                            else
                            {
